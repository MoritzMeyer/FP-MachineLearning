...
with gzip.open(testDataPath, 'rb') as f:
  ...
  testlBox = tmp["trainlBox"]
  testlLane = tmp["trainlLane"]
  testd = tmp["traind"]

  # check input shapes
  print(testlBox.shape)
  ...
  #check box values for nan vakzes
  print(np.any(np.isnan(testlBox)));

  # check min and max of box values
  print(testlBox.min(axis = 0).min());
  print(testlBox.min(axis = 0).max());
  print(testlBox.max(axis = 0).min());
  print(testlBox.max(axis = 0).max());
...
with tf.Session() as sess:
  ## input layer: Nx108x192x1
  dataPlaceholder = tf.placeholder(tf.float32, shape=[None, 108, 192, 1]);
  labelPlaceholder = tf.placeholder(tf.float32,[None, 8]) ;
  boxPlaceholder = tf.placeholder(tf.float32, shape=[None, 8]);

  ## Hidden Layer 1: 104x188x6 => 52x94x6
  ...
  ## Hidden Layer 2: 48x90x16 => 24x45x16
  conv2 = tf.nn.relu(tf.layers.conv2d(a1, 16, 5,name="H2")) ;
  a2 = tf.layers.max_pooling2d(conv2, 2, 2) ;
  print (a2) ;
  a2flat = tf.reshape(a2, (-1, 24 * 45 * 16)) ;

  # Den Vector der BoundingBox anhängen => 24x45x16 + 8
  a2flat = tf.concat([a2flat, boxPlaceholder], axis=1);

  ## Hidden Layer 3
  Z3 = 120 ;
  # allocate variables
  W3 = tf.Variable(np.random.uniform(-0.01,0.01, [24 * 45 * 16 + 8,Z3]),dtype=tf.float32, name ="W3") ; # adjust weights for a2flat!
  ...
  for iteration in range(0,tMax):
    ...
    if iteration%50==0:
      ...
      testacc = sess.run(nrCorrect, feed_dict = {dataPlaceholder: testd, labelPlaceholder: testlLane, boxPlaceholder: testlBox});
